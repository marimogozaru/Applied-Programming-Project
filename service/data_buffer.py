...
class EmgDataBuffer:
    ...
    def add_bytes(self, raw_bytes: bytes) -> None:
        self._byte_buffer.extend(raw_bytes)
        self._process_byte_buffer()

    def _process_byte_buffer(self) -> None:
        packets = []
        ...
        while len(self._byte_buffer) >= self._packet_size_bytes:
            ...
            packet = packet.reshape((self.n_channels, self.samples_per_packet))
    ...
    def get_channel(self, channel_index: int) -> np.ndarray:
        all_data = self.get_all_channels()
